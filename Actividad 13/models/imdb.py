...
class IMDb:
    ...
    def search_titles(self, title) -> dict:
        ...
        resultados = requests.get(
            f"https://imdb-api.com/API/SearchTitle/{self.apikey}/{title}"
        )
    ...
    def movie_reviews(self, imdb_id: str) -> dict:
        ...
        resultados = requests.get(
            f"https://imdb-api.com/API/Reviews/{self.apikey}/{imdb_id}"
        )
        if resultados.status_code == 200:
            return resultados.json()
        return {}
    ...
    def movie_ratings(self, imdb_id: str) -> dict:
        ...
        resultados = requests.get(
            f"https://imdb-api.com/API/Ratings/{self.apikey}/{imdb_id}"
        )
        if resultados.status_code == 200:
            return resultados.json()
        return {}
